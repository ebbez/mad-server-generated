...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'from'> & {
  from?: string | null;
};
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...customer,
      from: customer.from?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...restCustomer,
      from: restCustomer.from ? dayjs(restCustomer.from) : undefined,
    };
  }
