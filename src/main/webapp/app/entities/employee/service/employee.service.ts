...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'from'> & {
  from?: string | null;
};
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      from: employee.from?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      from: restEmployee.from ? dayjs(restEmployee.from) : undefined,
    };
  }
